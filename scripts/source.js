...
const star_star_creator = function () {
	...
	(this.render = function () {
		Draw.Rect(
			...
			this.w,
			this.h,
			TwoD_color_palettes.wicked_skies.yellow,
			1
		);
	}),
...
const star_update = function () {
	...
	stars_stars = stars_stars.filter((st) => {
		return st.active;
	});
	stars_stars.forEach((st) => {
		st.render(), st.update();
	});
...
const app = new RenderWindow(
	innerWidth,
	innerHeight,
	TwoD_color_palettes.wicked_skies.black,
	950
);
const ctx = app.CTX();
...
const hero = {
		...
		if (this.gunCld <= 0) {
			bullets.push(
				new Bullet(
					...
					this.y,
					'PLAYER',
					TwoD_color_palettes.wicked_skies.sky_blue
				)
			);
			this.gunCld = 100;
		}
	...
	render: function () {
		...
		Draw.Rect(
			...
			this.hpRectW,
			10,
			TwoD_color_palettes.wicked_skies.red
		); //HP čtverec
		Draw.text(
			ctx,
			'score:' + this.score,
		...
		Draw.text(
			...
			30,
			500,
			TwoD_color_palettes.wicked_skies.orange
		); //nejvyšší skore od poslední hry
		Draw.Rect(
			ctx,
			this.x - this.w / 2,
			this.y - 205,
			100,
			10,
			TwoD_color_palettes.wicked_skies.light_yellow
		); //míření
		Draw.Rect(
			ctx,
			this.x + this.w / 2 - 5,
...
class Enemy {
	...
	render = function () {
		Draw.text(
			...
			20,
			50,
			TwoD_color_palettes.wicked_skies.red_ish
		); // jmenovka nad nepřítelem
	...
	update = function () {
		...
		if (
			Distance(hero, { x: this.x, y: this.y, w: this.w, h: this.h }) <
				300 &&
			waves >= 10
		)
			this.shoot();
		...
		if (
			...
			//pokud je nepřítel mimo hrací pole -->zabí nepřítele
			this.active = false;
	};
}
//--------------------------------------------------
...
class Bullet {
	...
	update = function () {
		...
		else if (this.owner == 'ENEMY' && waves >= 50) {
			//pokud je vlastník nepřitel a počet vln přesáhl 50, potom pronásleduj hráče
...
const update = function (dt) {
	...
	if (Math.random() < 0.04) {
		//náhoda menší než .04 ? --> vytvoř nové instance pro hvězdu a sbíratelné
		stars.push(new Star());
...
const render = function () {
	app.Clear(
		...
		innerHeight,
		TwoD_color_palettes.wicked_skies.black
	); //vyčisti okno před novým kreslením
...
const changeStoreContent = function (what) {
	if (what == 'skins') {
		...
		document.querySelector(
			'.store__content__headline'
		).innerHTML = `Upgrades`; //změn obsah
		document.querySelector('.store__content__content').innerHTML = `
